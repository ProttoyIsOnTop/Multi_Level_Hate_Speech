...
Hate Presence (Hate Speech vs. Not Hate Speech)

Hate Type (e.g., Slander, Call to Violence, Religious Hate)

Target Category (e.g., Individual, Male, Female, Group)
...
Note: Due to privacy and ethical constraints, the raw dataset is not included in this repository. Please ensure you place your train.csv, val.csv, and test.csv in the appropriate directory before running the notebook.
...
The project relies on specific library versions to ensure compatibility between Unsloth, TRL, and Transformers. Run the following to install the core packages:
...
Model Initialization: Loads Meta-Llama-3.1-8B-Instruct in 4-bit precision.

LoRA Injection: Attaches trainable rank-decomposition matrices to the attention blocks.

Prompt Formatting: Maps dataset columns into the rigorous System Directive and JSON Expected Output format.

SFT Training: Executes Supervised Fine-Tuning using trl.SFTTrainer.

Checkpointing: Saves the optimized adapters to your specified directory.
